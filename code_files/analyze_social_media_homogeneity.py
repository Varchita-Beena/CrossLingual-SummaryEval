...
INPUT_ROOTS = {
    "openai": BASE_DIR / "openai" / "embedding" / "social_media",
    "sarvam": BASE_DIR / "sarvam" / "embedding" / "social_media",
}
LANGUAGES = ("english", "hindi")
RUNS = ("1", "2", "3")
OUTPUT_DIR = BASE_DIR / "analysis" / "social_media_homogeneity"
...
def load_run(model, language, run):
    ...
    for path in sorted(folder.glob("*.json"), key=lambda p: int(p.stem)):
        ...
        items[data["id"]] = {
            "file_name": path.name,
            "headline": data["social_media_headline"],
...
def compare_records(left, right):
    left_headline = left["headline"] or ""
    right_headline = right["headline"] or ""
    left_tokens = tokenize(left_headline)
    right_tokens = tokenize(right_headline)
    ...
    return {
        ...
        "exact_headline_match": int(left_headline == right_headline),
        "left_word_count": len(left_tokens),
        "right_word_count": len(right_tokens),
        "left_char_count": len(left_headline),
        "right_char_count": len(right_headline),
        "word_count_delta": abs(len(left_tokens) - len(right_tokens)),
        "char_count_delta": abs(len(left_headline) - len(right_headline)),
    }
...
def build_group_metrics(records_by_label):
    ...
    all_word_counts = {
        label: len(tokenize(record["headline"]))
        for label, record in records_by_label.items()
    }
    all_char_counts = {
        label: len(record["headline"] or "")
        for label, record in records_by_label.items()
    }
...
def summarize_pair_rows(pair_rows):
    ...
    exact_matches = [row["exact_headline_match"] for row in pair_rows]
...
def main():
        ...
        for model in INPUT_ROOTS:
            for run in RUNS:
                for article_id, record in data[model][language][run].items():
                    metrics = lexical_metrics(record["headline"] or "")
                    lexical_rows.append({
                        "language": language,
                        "model": model,
                        "run": run,
            ...
            for article_id in common_ids:
                ...
                for pair_row in group_metrics["pair_rows"]:
                    detail_rows.append({
                        ...
                        "left_headline": records[pair_row["left_label"]]["headline"],
                        "right_headline": records[pair_row["right_label"]]["headline"],
                        **pair_row,
                    })
                    boxplot_rows.append({
                        "comparison_scope": "intra_model",
                        ...
                        "value": pair_row["cosine_similarity"],
                    })
                    boxplot_rows.append({
                        "comparison_scope": "intra_model",
                        "language": language,
                        "model_or_pair": model,
                        "article_id": article_id,
                        "metric": "word_jaccard_similarity",
                        "value": pair_row["word_jaccard_similarity"],
                    })
                    boxplot_rows.append({
                        "comparison_scope": "intra_model",
                        "language": language,
                        "model_or_pair": model,
                        "article_id": article_id,
                        "metric": "word_count_delta",
                        "value": pair_row["word_count_delta"],
                    })
                    boxplot_rows.append({
                        "comparison_scope": "intra_model",
                        "language": language,
                        "model_or_pair": model,
                        "article_id": article_id,
                        "metric": "char_count_delta",
                        "value": pair_row["char_count_delta"],
                    })
        ...
        for article_id in common_ids:
            ...
            for openai_run, sarvam_run in product(RUNS, RUNS):
                ...
                detail_rows.append({
                    ...
                    "left_headline": records[left_label]["headline"],
                    "right_headline": records[right_label]["headline"],
                    **pair_row,
                })
                boxplot_rows.append({
                    "comparison_scope": "inter_model",
    ...
    write_csv(
        OUTPUT_DIR / "headline_variation.csv",
        variation_rows,
        [
            "comparison_scope",
            "language",
    ...
    report_lines = [
        "# Social Media Homogeneity Report",
        "",
        "This report compares headline embeddings and headline text variation across repeated runs.",
        "",
        "## What is measured",
        "",
        ...
        "- Cosine similarity: checks how close the embeddings are.",
        "- Word Jaccard similarity: checks token overlap between two headlines.",
        "- Exact match rate: fraction of pairwise comparisons where the headline text is identical.",
        ...
        "- `headline_variation.csv`: longest/shortest headline diagnostics for each article.",
        "- `centroid_analysis.csv`: per-run distance and cosine against the article centroid.",
        "- `lexical_diversity.csv`: per-headline lexical diversity metrics.",
    ...
    print(f"  - {OUTPUT_DIR / 'headline_variation.csv'}")
