...
BASE_DIR = Path("output/analysis/nearest_text_consistency")
INPUT_CSV = BASE_DIR / "pairwise_details.csv"
...
DISPLAY_NAMES = {
    ...
    "bigram_jaccard_similarity": "Bigram Jaccard Similarity",
    "trigram_jaccard_similarity": "Trigram Jaccard Similarity",
...
def group_values(rows, language, comparison_scope, metric):
    ...
    for row in rows:
        ...
        if value == "":
            continue
        grouped[row["model_or_pair"]].append(float(value))
    return grouped
...
def save_boxplot(language, comparison_scope, metric, grouped_values):
    ...
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / f"{language}_{comparison_scope}_{metric}_boxplot.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def save_histogram(language, comparison_scope, metric, grouped_values):
    ...
    ax.grid(axis="y", linestyle="--", alpha=0.4)
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / f"{language}_{comparison_scope}_{metric}_histogram.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    summary_path.write_text(
        "\n".join(
            [
                "# Nearest Text Consistency Plots",
                "",
                "This folder contains boxplots and histograms generated from `pairwise_details.csv`.",
                "",
                "Metrics:",
                "",
